...
// Standalone SHA-256 implementation (Simplified for Demo)
// This ensures the code compiles with zero external dependencies
typedef struct {
    uint8_t data[64];
    uint32_t datalen;
    uint64_t bitlen;
...
int hash_file_content(const char *filename, char *output_hash) {
    ...
    sha256_final(&ctx, hash);
    fclose(fp);
    for (int i = 0; i < 32; i++) sprintf(output_hash + (i * 2), "%02x", hash[i]);
    output_hash[64] = '\0';
    return 1;
}

    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div className="space-y-1">
          ...
          <p className="text-slate-500">Deep inspection of the blockchain state and cryptographic links</p>
        </div>
        ...
        {[0, 1, 2].map(i => (
          <div key={i} className="glass-card overflow-hidden">
            <div className="bg-slate-50 px-6 py-3 border-b border-slate-200 flex justify-between items-center">
              <span className="font-mono text-sm font-bold text-slate-600">BLOCK_HEIGHT: {i}</span>
              <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-bold">IMMUTABLE</span>
            </div>
            <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-100 space-y-3">
                ...
                <div className="pt-2">
                  <button className="w-full py-1.5 text-xs font-bold text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-50 transition-colors">
                    View raw data (Hex)
                  </button>
                </div>
              </div>
...
const DataField = ({ icon, label, value }: { icon: any, label: string, value: string }) => (
  <div className="space-y-1">
    ...
    <div className="bg-white border border-slate-200 rounded-lg px-3 py-2 font-mono text-xs text-slate-700 shadow-inner">
      {value}
    </div>
  </div>
);

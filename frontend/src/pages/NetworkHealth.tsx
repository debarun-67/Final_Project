...
const NetworkHealth: React.FC = () => {
  const nodes = [
    { port: 8001, status: 'online', type: 'Primary Validator', latency: '12ms' },
    { port: 8002, status: 'online', type: 'Backup Node', latency: '15ms' },
    { port: 8003, status: 'offline', type: 'Auditor Node', latency: '-' },
  ];
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-slate-500">Monitor distributed validator nodes and P2P synchronization status</p>
      </div>
          ...
          <div key={node.port} className={`glass-card p-6 border-l-4 ${
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm font-bold text-slate-700">{node.type}</p>
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <span className="text-xs font-mono font-bold text-slate-600">{node.latency}</span>
              </div>
            </div>
          </div>
        ))}
